...
describe("WatchPartyScreen", () => {
  ...
  it("should handle play/pause button press", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        WatchPartyService.getInstance().updatePlaybackState,
  ...
  it("should handle sending chat messages", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        WatchPartyService.getInstance().sendChatMessage,
  ...
  it("should handle video position updates", async () => {
    ...
    await waitFor(() => {
      ...
      fireEvent(video, "onPlaybackStatusUpdate", {
        isLoaded: true,
        positionMillis: 1000,
      });
      expect(
        WatchPartyService.getInstance().updatePlaybackState,
      ).toHaveBeenCalledWith(false, 1000);
    });
  });
