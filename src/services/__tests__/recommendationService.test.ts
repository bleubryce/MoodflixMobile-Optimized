...
describe("RecommendationService", () => {
  ...
  const mockWatchHistory = [{ movie_id: 1, mood: "excited" }];

  const mockFavorites = [{ movie_id: 1 }];
  ...
  describe("getPersonalizedRecommendations", () => {
    ...
    it("returns cached movies when offline or on error", async () => {
      ...
      (OfflineService.getCachedMovies as jest.Mock).mockResolvedValue(
        mockMovies,
      );

      const recommendations =
        await RecommendationService.getPersonalizedRecommendations("user123");
